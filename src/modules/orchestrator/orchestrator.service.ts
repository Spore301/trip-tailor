import type { Activity, FlightData, HotelData, PlaceData, FoodData, CarRentalData } from '../aggregator/types.js';
...
interface BudgetAllocation {
	flights: number; // Percentage of budget
	hotels: number;
	car_rentals: number;
	activities: number;
	food: number;
...
class OrchestratorService {
	 ...
	 */
	private getBudgetAllocation(experience: ExperienceType): BudgetAllocation {
		switch (experience) {
			case 'adventure':
		...
		if (experience === 'adventure') {
			...
			filtered = filtered
				.sort((a, b) => {
					...
					if (a.estimated_cost > 0 && b.estimated_cost === 0) return -1;
					if (a.estimated_cost === 0 && b.estimated_cost > 0) return 1;
					// Then by rating
			...
			filtered = filtered
				.sort((a, b) => {
					...
					const uniquenessScore = (place: PlaceData) => {
		...
		if (experience === 'adventure') {
			...
			filtered = filtered
				.sort((a, b) => {
					...
					const priceWeight = (f: FoodData) => f.price_level || 2;
			...
			filtered = filtered
				.sort((a, b) => {
					...
					const score = (f: FoodData) => (f.rating || 0) + (f.cuisine?.toLowerCase().includes('local') ? 1 : 0);
		...
		if (experience === 'adventure') {
			...
			filtered = filtered
				.sort((a, b) => {
					...
					const suvPriority = (r: CarRentalData) => (r.car_type?.toLowerCase().includes('suv') ? 0 : 1);
			...
			filtered = filtered
				.sort((a, b) => {
					...
					const luxuryPriority = (r: CarRentalData) =>
						r.car_type?.toLowerCase().includes('luxury') || r.car_type?.toLowerCase().includes('sedan') ? 0 : 1;
	 ...
	 */
	async aggregateAndFilter(params: TripRequestParams): Promise<FilteredTripData> {
		...
		if (totalCost > budget) {
			...
			const finalFlights = budgetFiltered.filter((a) => a.type === 'flight') as FlightData[];
			const finalHotels = budgetFiltered.filter((a) => a.type === 'hotel') as HotelData[];
			const finalCarRentals = budgetFiltered.filter((a) => a.type === 'car_rental') as CarRentalData[];
			const finalPlaces = budgetFiltered.filter((a) => a.type === 'attraction') as PlaceData[];
			const finalFood = budgetFiltered.filter((a) => a.type === 'food') as FoodData[];

...
class FoodService {
	private async fetchFromGooglePlaces(params: FoodSearchParams): Promise<FoodData[]> {
		...
		try {
			...
			const placeIds = response.data.results.slice(0, 10).map((r: any) => r.place_id);
	...
	private normalizeGooglePlacesFoodResponse(data: any[]): FoodData[] {
		return data.map((restaurant: any) => {
			...
			const cuisineTypes = restaurant.types?.filter((t: string) =>
				['restaurant', 'food', 'meal_takeaway', 'cafe'].includes(t) === false,
			);
	...
	private getMockFood(params: FoodSearchParams): FoodData[] {
		return [
			{
	...
	async searchFood(params: FoodSearchParams): Promise<FoodData[]> {
		// Check cache first
		const cacheKey = cacheService.generateKey('food', params);
		const cached = cacheService.get<FoodData[]>(cacheKey);
		if (cached) {
			return cached;
		}

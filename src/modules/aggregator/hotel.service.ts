...
class HotelService {
	private async fetchFromRapidAPI(params: HotelSearchParams): Promise<HotelData[]> {
	...
	private normalizeBookingResponse(data: any, params: HotelSearchParams): HotelData[] {
	...
	private getMockHotels(params: HotelSearchParams): HotelData[] {
	...
	async searchHotels(params: HotelSearchParams): Promise<HotelData[]> {
		// Check cache first
		const cacheKey = cacheService.generateKey('hotels', params);
		const cached = cacheService.get<HotelData[]>(cacheKey);
		if (cached) {
			return cached;
		}

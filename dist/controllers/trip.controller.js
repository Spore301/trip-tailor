...
export async function createTripRequest(req, res) {
    try {
        const { destination, people_count, days, budget, experience } = req.body;
        // Save trip request
        ...
        const tripRequestId = tr._id?.toString?.() ?? tr.id;
        ...
        const itineraryId = itinerary._id?.toString?.() ?? itinerary.id;
        ...
        return res.status(200).json({
            ...
            data: filteredData,
            summary: {
                totalActivities: filteredData.flights.length +
...
export async function getAllItineraries(_req, res) {
    try {
        const repo = AppDataSource.getRepository(Itinerary);

...
class ActivitiesService {
    async fetchFromBookingCom(params) {
        ...
        try {
            ...
            const response = await axios.get('https://booking-com15.p.rapidapi.com/api/v1/attractions/searchAttractions', {
                headers: {
    ...
    normalizeBookingComResponse(data, params) {
        if (!data.data || !Array.isArray(data.data)) {
            return [];
        }
        return data.data.slice(0, 15).map((activity) => {
            // Extract price

...
class PlacesService {
    async fetchFromGooglePlaces(params) {
        ...
        try {
            ...
            const placeIds = textSearchResponse.data.results.slice(0, 10).map((r) => r.place_id);
            const placesDetails = await Promise.all(placeIds.map((placeId) => axios.get('https://maps.googleapis.com/maps/api/place/details/json', {
                params: {
                    place_id: placeId,
                    key: apiKey,
    ...
    normalizeGooglePlacesResponse(data) {

...
router.get('/flights', async (req, res) => {
    try {
        ...
        if (returnDate)
            flightParams.returnDate = String(returnDate);
        if (adults)
            flightParams.adults = Number(adults);
...
router.get('/hotels', async (req, res) => {
    try {
        ...
        if (budget)
            hotelParams.budget = Number(budget);
...
router.get('/places', async (req, res) => {
    try {
        ...
        if (type)
            placesParams.type = String(type);
        if (budget)
            placesParams.budget = Number(budget);
...
router.get('/food', async (req, res) => {
    try {
        ...
        if (budget)
            foodParams.budget = Number(budget);
...
router.get('/car-rentals', async (req, res) => {
    try {
        ...
        if (pickUpTime)
            carRentalParams.pickUpTime = String(pickUpTime);
        if (dropOffTime)
            carRentalParams.dropOffTime = String(dropOffTime);
        if (driverAge)
            carRentalParams.driverAge = Number(driverAge);
        if (budget)
            carRentalParams.budget = Number(budget);
...
router.get('/activities', async (req, res) => {
    try {
        ...
        if (budget)
            activitiesParams.budget = Number(budget);
...
//# sourceMappingURL=aggregator.routes.js.map